#Chrome profile path: C:\Users\HungTran\AppData\Local\Google\Chrome\User Data\Profile 2
...
def create_chrome_driver():
    ...
    #Profile:
    options.add_argument(r'--user-data-dir=C:/Users/HungTran/AppData/Local/Google/Chrome/User Data')
    options.add_argument(r'--profile-directory=Profile 2')
    return uc.Chrome(options=options,version_main=139)
...
def loadDataPage():
    ...
    r = requests.get(f'https://surfearner.com/cpa', headers=headers, cookies=cookies)
    soup = BeautifulSoup(r.text, 'html.parser')
    links = []
    times = []
    href_list_find = soup.find_all('a',class_='title nodecor clip')
    href_list = [a.get('href') for a in href_list_find]
    time_list_find = soup.find_all("font", attrs={"dir": "auto", "style": "vertical-align: inherit;"})
    time_list = [e.get_text(strip=True) for e in time_list_find]
    reward_list_find = soup.find_all("font", attrs={"dir": "auto", "style": "vertical-align: inherit;"})
    reward_list = [parse_currency(e.get_text(strip=True)) for e in reward_list_find]
    text_list = [e.get_text(strip=True) for e in soup.find_all("font", attrs={"dir": "auto", "style": "vertical-align: inherit;"})]
    with open('list.txt','w',encoding='utf-8') as f:
        for a in range(len(time_list)-1):
            f.write(f'{href_list[a]},{time_list[a]},{reward_list[a]},{text_list[a]}')

"""
    for a in range((text_list)-1):
        text = text_list[a]
        if 'Join' in text or 'Search' in text or 'Transition' in text or 'Registration' in text or 'Repost' in text or 'Like' in text or 'Telegram' in text or 'Publish' in text or 'Subscribe' in text:
            pass
"""
...
print(f'{lgreen}Success create chrome driver')
wait = WebDriverWait(driver, 10)  # tối đa 10 giây
actions = ActionChains(driver)

print(f'{lgreen}Getting full data from surfearner')
loadDataPage()
print(f'{lgreen}Done!')
